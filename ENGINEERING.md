...
接口重点是将Sql语句，如果是新增则删除，更新则更新，删除则添加。

所以很重要的是，需要记住每一次sql语句执行操作的影响及标识。

当新增时，则需要记住新增之后返回的表主键。

当删除，则需要记住被删除数据的主键。

当更新，则需要记住一份完整的，更新前的数据。

难点：

1、记录更新前的数据，是以sql保存还是提取，转换为对象。

2、复杂sql，不能简简单单的依靠字符串去判断sql操作。
